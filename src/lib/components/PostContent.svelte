...
{#if showContent[TOTAL_ELEMENTS_TO_TRANSITION - 1]}
	<div class="text-secondary absolute top-0 -right-80 hidden h-full w-72 p-4 xl:block" in:fade>
...
<div class="p-8">
	{#if showContent[0]}
		<div class="text-secondary text-sm" in:fade>
			{metadata.date}
			...
			<div class="flex flex-wrap" in:fade>
				{#each metadata.tags as tag}
					<a
						...
						class="not-prose text-secondary hover:text-accent p-2"
					>
						#{tag}
					</a>
				{/each}
...
{#if showContent[3]}
	<div class="p-8 py-0 text-right" in:fade>
		<button
			onclick={scrollToTop}
			class="text-secondary sm:hover:text-accent cursor-pointer text-base"
		>
			Back to Top
		</button>
	</div>
