...
- An [AWS account](https://aws.amazon.com/): With appropriate permissions to create S3 buckets, Lambda functions, and API Gateway endpoints. It is recommended to avoid using the root user and to [use an admin user instead](/blog/getting-started-with-aws-core-concepts-and-iam#create-a-user-and-a-group).
...
- Basic understanding of Python is also recommended, our (very simple) Lambda function will be written in Python.
...
2. **Upload Website Content:**
    ...
    - In the S3 view of the AWS Management Console, click "Upload", then "Add files", and select `index.html` and `error.html`.
    - Click "Upload".
